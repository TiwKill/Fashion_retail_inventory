...
export function StoreSimulation({
    ...
    const [currentStock, setCurrentStock] = useState<{ [brand: string]: number }>({})
    ...
    const dailySalesProgressRef = useRef<{ [brand: string]: number }>({})
    const dailyTargetStockRef = useRef<{ [brand: string]: number }>({})
    ...
    // Initialize stats when selectedBrands changes
    useEffect(() => {
    ...
    const getShelfPositions = useCallback(() => {
    ...
    const getDailyDataForDay = useCallback((dayInMonth: number) => {
        ...
        const dailyDataForBrands: { [brand: string]: DailyData | undefined } = {}

        for (const brand of selectedBrands) {
            dailyDataForBrands[brand.toLowerCase()] = dailyData.find((d) => d.day === absoluteDay && d.brand === brand)
        }

        return dailyDataForBrands
    }, [availableMonths, selectedMonth, simulationDays, dailyData, selectedBrands])

    const getStartingStockForDay = useCallback((dayInMonth: number) => {
        if (dayInMonth === 1) {
            const daysPerMonth = Math.floor(simulationDays / availableMonths.length)
            const absoluteDay = selectedMonth * daysPerMonth - 1

            if (absoluteDay >= 0) {
                const startingStockForBrands: { [brand: string]: number } = {}
                ...
                for (const brand of selectedBrands) {
                    ...
                    startingStockForBrands[brand.toLowerCase()] = previousDayData?.stock_after || fallbackStock
                }
        ...
        for (const brand of selectedBrands) {
            const brandConfig = configs[brand]
            const fallbackStock = brandConfig?.initial_stock || 4000
            startingStockForBrands[brand.toLowerCase()] = previousDayData[brand.toLowerCase()]?.stock_after || fallbackStock
        }

        return startingStockForBrands
    }, [availableMonths, selectedMonth, simulationDays, dailyData, selectedBrands, configs, getDailyDataForDay])

    const getDailySalesInfo = useCallback((dayInMonth: number) => {
        const currentDayData = getDailyDataForDay(dayInMonth)
        const startingStock = getStartingStockForDay(dayInMonth)

        const dailySalesInfoForBrands: {
    ...
    }, [getDailyDataForDay, getStartingStockForDay, selectedBrands])

    const getMonthlyStockLevels = useCallback(() => {
        return getStartingStockForDay(1)
    }, [getStartingStockForDay])

    const getMonthlySales = useCallback(() => {
        const actualMonth = availableMonths[selectedMonth]
        const monthData = monthlyData.filter((d) => d.month === actualMonth)
        const monthlySalesForBrands: { [brand: string]: number } & { total?: number } = {}
    ...
    }, [availableMonths, selectedMonth, monthlyData, selectedBrands])

    const getMonthlyRestockEvents = useCallback(() => {
        const daysPerMonth = Math.floor(simulationDays / availableMonths.length)
    ...
    }, [availableMonths, selectedMonth, simulationDays, restockEvents])
    ...
    const drawStore = useCallback((ctx: CanvasRenderingContext2D) => {
        ...
        for (const brand of selectedBrands) {
            ...
            const brandColor = getStockColor(currentStock[brand.toLowerCase()] || 0)
            ...
            ctx.fillText(`Stock: ${currentStock[brand.toLowerCase()] || 0}`, shelf.x, shelf.y + 10)
        }
    ...
    }, [shelfPositions, currentStock, selectedBrands])
    ...
    const drawEmployee = (ctx: CanvasRenderingContext2D, employee: Employee) => {
        const baseColor = employee.color
        const employeeColor = baseColor.replace(/[0-9a-f]{2}$/i, (match) => {
    ...
    const spawnCustomer = useCallback(() => {
        const availableBrands = selectedBrands.filter((brand) => (currentStock[brand.toLowerCase()] || 0) > 0)

        if (availableBrands.length === 0) {
            return
        }

        const totalWeight = availableBrands.reduce((sum, brand) => sum + (monthlySales[brand.toLowerCase()] || 0), 0)
        ...
        for (const brand of availableBrands) {
            random -= monthlySales[brand.toLowerCase()] || 0
            if (random <= 0) {
                selectedBrand = brand
                break
            }
    ...
    }, [selectedBrands, currentStock, monthlySales, brandColors])

    const spawnEmployee = useCallback((brand: string, restockQuantity: number) => {
        const shelf = shelfPositions[brand]
        if (!shelf) return
    ...
    }, [shelfPositions, brandColors])

    const updateCustomer = useCallback((customer: Customer): Customer => {
            ...
            switch (customer.state) {
                ...
                case "checkout":
                    if (!customer.hasPurchased) {
                        ...
                        const totalDailySales = brandInfo?.sales || 0
                        const salesSoFar = dailySalesProgressRef.current[brandKey] || 0
                        const remainingSales = totalDailySales - salesSoFar

                        const customersForBrand = customersRef.current.filter((c) => c.brand === customer.brand && !c.hasPurchased).length
                        const stockDecrease = Math.max(
                            1,
                            Math.ceil(remainingSales / Math.max(1, customersForBrand)),
                        )

                        dailySalesProgressRef.current[brandKey] = (dailySalesProgressRef.current[brandKey] || 0) + stockDecrease

                        setCurrentStock((prev) => ({
                            ...prev,
                            [brandKey]: Math.max(0, (prev[brandKey] || 0) - stockDecrease),
                        }))
                    }
                    return {
                        ...customer,
    ...
    }, [shelfPositions, currentDay, getDailySalesInfo])

    const updateEmployee = useCallback((employee: Employee): Employee => {
            ...
            switch (employee.state) {
                ...
                case "restocking":
                    ...
                    if (!employee.hasRestocked) {
                        setCurrentStock((prev) => ({
                            ...prev,
                            [employee.brand.toLowerCase()]: (prev[employee.brand.toLowerCase()] || 0) + employee.restockQuantity,
                        }))
                    }
                    return {
                        ...employee,
    ...
    const animate = useCallback(
        (timestamp: number) => {
            ...
            const daysPerMonth = Math.floor(simulationDays / availableMonths.length)
            const shouldUpdateDay = timestamp - lastDayUpdateRef.current > DAY_DURATION && !isMonthComplete
            ...
            if (shouldUpdateDay) {
                ...
                if (nextDay > daysPerMonth) {
                    ...
                    // ตรวจสอบว่าค่าเปลี่ยนแปลงก่อนเรียก setState
                    ...
                    dailySalesProgressRef.current = {}
                    dailyTargetStockRef.current = {}
        ...
        [
            ...
            monthlySales,
            isMonthComplete,
            currentDay, // เพิ่ม currentDay ใน dependencies
            availableMonths,
            selectedBrands,
            simulationDays,
            drawStore,
    ...
    }, [currentStock, configs, selectedBrands, drawStore])

    useEffect(() => {
        const startingStock = getStartingStockForDay(1)
        const dailySalesInfo = getDailySalesInfo(1)

        // ตรวจสอบว่า currentStock แตกต่างจริงๆ ก่อนอัพเดท
        const shouldUpdateStock = Object.keys(startingStock).some(
            brand => startingStock[brand] !== currentStock[brand]
        )
        ...
        // อัพเดท refs โดยตรงแทนการใช้ state สำหรับค่าที่ไม่ต้อง trigger re-render
        for (const brand of selectedBrands) {
            dailyTargetStockRef.current[brand.toLowerCase()] = dailySalesInfo[brand.toLowerCase()]?.targetStock || 0
            dailySalesProgressRef.current[brand.toLowerCase()] = 0
        }
    ...
    }, [selectedMonth, selectedBrands, getStartingStockForDay, getDailySalesInfo, currentStock])
    ...
    }, [selectedBrands, getStartingStockForDay, getDailySalesInfo])
    ...
    }, [availableMonths, selectedMonth, bestSellingProducts, selectedBrands])
    ...
    const jumpToDay = useCallback(
        ...
        [simulationDays, availableMonths, selectedBrands, getStartingStockForDay, getDailySalesInfo]
    )

    const handleDayChange = useCallback((value: number[]) => {
        const targetDay = value[0]
        // ตรวจสอบว่าเป็นวันใหม่จริงๆ ก่อนเรียก jumpToDay
        if (targetDay !== currentDay) {
            jumpToDay(targetDay)
        }
    }, [currentDay, jumpToDay])
    ...
    }, [simulationDays, availableMonths, selectedMonth, currentDay, dailyData])
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <CardDescription className="mt-1.5">
                            Watch customers shop and employees restock shelves in real-time
                        </CardDescription>
                    </div>
            ...
            <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
                    <div className="rounded-lg border border-border bg-muted/50 p-3">
                        ...
                        <div className="text-2xl font-bold">{(monthlySales.total || 0).toLocaleString()}</div>
                        <div className="text-xs text-muted-foreground mt-1">Total units sold</div>
                    </div>
                    {selectedBrands.map((brand) => (
                        <div key={brand} className="rounded-lg border border-border bg-muted/50 p-3">
                            ...
                            <div className="text-2xl font-bold" style={{ color: brandColors[brand] }}>
                                {(monthlySales[brand.toLowerCase()] || 0).toLocaleString()}
                            </div>
                            <div className="text-xs text-muted-foreground mt-1">
                                Stock: {currentStock[brand.toLowerCase()] || 0}
                            </div>
